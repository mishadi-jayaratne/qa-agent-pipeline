...
description: Optional agent. Extracts requirement/acceptance-criteria items tagged with their CR/requirement ID from an SRS or CR document, cross-references them against changes.md, and flags mismatches against code-scan.md's discovered business rules if that stage has run. Seeds the Requirement Traceability Matrix (rtm.md/rtm.csv) for the run. Use when a requirements/CR document is available for this release; skips itself plainly if not.
